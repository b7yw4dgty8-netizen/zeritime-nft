#!/usr/bin/env python3
"""Build a 100% Repentance save and iOS placement variants."""
...
CHARACTERS = [
    ...
    "Jacob & Esau", "T Isaac", "T Maggy", "T Cain", "T Judas", "T ???", "T Eve", "T Samson",
    "T Azazel", "T Lazarus", "T Eden", "T Lost", "T Lilith", "T Keeper", "T Apollyon",
    "T Forgotten", "T Bethany", "T Jacob",
]
...
def alter_int(data: bytearray, offset: int, new_val: int, num_bytes: int = 2) -> None:
...
def update_checklist_unlocks(data: bytearray, char_index: int, marks: list[int]) -> None:
    if char_index == 14:
        ...
        for i, mark in enumerate(marks):
            ...
            alter_int(data, current_ofs, mark, 4)
            if i == 8:
                clu_ofs += 0x4
            if i == 9:
                clu_ofs += 0x37C
...
def build_full_save(source: bytes) -> bytearray:
    ...
    alter_int(data, stats + 0x4C, 999, 4)   # donation machine
    alter_int(data, stats + 0x50, 50000, 4)  # total donated
    alter_int(data, stats + 0x54, 99, 4)    # eden tokens
    alter_int(data, stats + 0x58, 99, 4)    # win streak
    alter_int(data, stats + 0x58 + 4, 99, 4)  # best streak
...
def summarize(data: bytes | bytearray) -> None:
    ...
    clu_ofs = sections[1] + 0x6C
    isaac_marks = [
        int.from_bytes(data[clu_ofs + i * 14 * 4 : clu_ofs + i * 14 * 4 + 4], "little")
        for i in range(12)
    ]
    print(f"  size={len(data)} header={data[2:18]!r}")
    print(f"  secrets={secrets}/637 items={items}/732 challenges={challenges}/45")
    print(f"  Isaac marks={isaac_marks}")


def main() -> None:
    if not SRC.exists():
        raise SystemExit(f"Missing template save: {SRC}")

    source = SRC.read_bytes()
    full = build_full_save(source)
    OUT_DIR.mkdir(exist_ok=True)

    pc_path = OUT_DIR / "rep_100_full.dat"
    pc_path.write_bytes(full)
    print("PC 100% save:")
    summarize(full)

    variants = {
        "ios_pc_direct.dat": full,
        "ios_pc_f30a_prefix.dat": b"\xf3\x0a" + full,
        "ios_pc_06r_header.dat": bytearray(full),
    }
    header_06r = b"ISAACNGSAVE06R  "
    variants["ios_pc_06r_header.dat"][2:18] = header_06r

    if IOS_SAVE.exists():
        backup = IOS_SAVE.with_suffix(".dat.empty.bak")
        shutil.copy2(IOS_SAVE, backup)
        print(f"\nBacked up current iOS save -> {backup}")

    # Primary install: standard Repentance PC dead-god layout (what iOS engine may accept)
    IOS_SAVE.write_bytes(full)
    print(f"\nInstalled primary save -> {IOS_SAVE} ({len(full)} bytes)")

    for name, payload in variants.items():
        path = OUT_DIR / name
        path.write_bytes(payload)
        print(f"Wrote variant {name} ({len(payload)} bytes)")
